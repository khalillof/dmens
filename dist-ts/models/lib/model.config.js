...
class ModelConfig {
    constructor(_config) {
        ...
        if (index_js_1.Store.db.exist(this.name)) {
        ...
        this.dependent = _config.dependent || false,
            ...
            this.routeName = _config.routeName ? _config.routeName.replace('/', '').toLowerCase() : index_js_1.Store.route.pluralizeRoute(this.name);
    ...
    getRoutes() {
        return index_js_1.Store.route.getRoutesPathMethods(this.routeName);
    }
    async genForm() {
        if (this.formCache)
            return this.formCache;
        let _form = new index_js_2.ModelForm(this);
