...
export class ClientSeedDatabase {
    ...
    async init() {
        this.addRoles().then(()=> console.log('finished seeding roles'))
         ...
         .then(()=>this.addCateories().then(()=> console.log('finished seeding categoriess'))));
    ...
    async addCommentsToPosts() {
      let posts_ids =await this.getIDs('post'); 
            ...
            posts_ids.forEach(async(post_id)=>{  

                let comments_ids = await this.getIDs('comment',{ post_id: post_id });

                comments_ids?.length && await dbStore['post'].putById(post_id, { comments: comments_ids });
             })                             
            }
    }
